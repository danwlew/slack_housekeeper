...
def send_archived_notification(channel_id, channel_name):
    archive_notification = (
        f"The channel #{channel_name} has been archived because it was unused :( . If you need it reopened, please contact an admin."
    )
...
def remove_archived_channels():
    ...
    if response["ok"]:
        ...
        for channel in channels:
            if channel.get("is_archived") and datetime.now() - timedelta(days=90) > convert_unix_timestamp_to_datetime(
                    channel.get("created")):
                response = client.conversations_delete(channel=channel["id"])
                if response["ok"]:
                    print(f"Archived channel {channel['name']} removed successfully.")
                    send_notification_to_housekeeping(channel["name"], f"Channel {channel['name']} has been removed, because it was unused for 90 days.")
                else:
                    print(
                        f"Error removing archived channel {channel['name']}: {response.get('error', 'Unknown error')}")
    else:
...
for channel in channel_list_with_last_message:
    ...
    if last_message:
        ...
        if time_duration > timedelta(hours=2):
            ...
            send_notification_to_housekeeping(channel["name"], f"Channel {channel['name']} is inactive.")
